...
ashita.events.register('d3d_present', 'present_cb', function()
    ...
    local render = targetId ~= nil and targetTable[targetId] and
    ...
    if render or chains.visible or chains.position then
        ...
        if chains.position then
            ...
            imgui.SetNextWindowPos({ chains.settings.position_x, chains.settings.position_y }, ImGuiCond_Appearing,
                { 0, 0 });
        end
...
ashita.events.register('command', 'command_cb', function(e)
    ...
    if (#args == 2) and chains.settings.display:containskey(args[2]) then
        ...
        print(chat.header(addon.name):append(chat.message(outText):fmt(args[2],
            chains.settings.display[args[2]] and 'on' or 'off')));
    end
    ...
    if (#args == 4) and (args[2] == 'move') then
        ...
        print(chat.header(addon.name):append(chat.message('Window position set to x: %s, y: %s'):fmt(chains.position.x,
            chains.position.y)));
    end
end);
